...
public class TeamCreateActivity extends BaseActivity implements TeamContract{

    private String selectTeamImagePath;
    private String selectTeamImageUrl;
    @BindView(R.id.team_logo)
    ImageView team_logo;
    @BindView(R.id.input_team_name)
    EditText input_team_name;
    @BindView(R.id.input_team_abstract)
    EditText input_team_abstract;
    @BindView(R.id.input_team_detail)
    EditText input_team_detail;
    ...
    @OnClick({R.id.select_logo_text,R.id.team_logo})
    void select_image(){
...
//                .recordVideoSecond()//视频秒数录制 默认60s int
    ...
    @OnClick(R.id.footer)
    void submit(){
        team_name = input_team_name.getText().toString();
        team_abstract = input_team_abstract.getText().toString();
        team_detail = input_team_detail.getText().toString();
        if (!StringUtil.isNotEmpty(selectTeamImagePath)){
            CommonTipsDialog.showTip(this,"温馨提示","请选择团队logo",false);
        }else if (!StringUtil.isNotEmpty(team_name)){
            CommonTipsDialog.showTip(this,"温馨提示","请输入团队名字",false);
        }else if (!StringUtil.isNotEmpty(team_abstract)){
            CommonTipsDialog.showTip(this,"温馨提示","请输入团队简介",false);
        }else if (!StringUtil.isNotEmpty(team_detail)){
            CommonTipsDialog.showTip(this,"温馨提示","请输入团队详细介绍",false);
        }else{
            setLoadingEnable(true);
            uploadFile(selectTeamImagePath);
        }
    }
    ...
    private void submitData(){
        if (teamPresenter!=null){
            ...
            params.put("team_leader", AccountTool.getLoginUser(context).getUser_id());
            params.put("team_create_time",StringUtil.getTime());
            params.put("team_create_user",AccountTool.getLoginUser(context).getUser_id());
            params.put("team_name",team_name);
            params.put("team_detail",team_detail);
            params.put("team_abstract",team_abstract);
            params.put("team_status","1");
            params.put("team_logo",selectTeamImageUrl);
            Log.e("submitData",""+params.toString());
            teamPresenter.createTeam(params);
        }
    }
        ...
        switch (requestCode) {
            case PictureConfig.CHOOSE_REQUEST:
                ...
                if (selectList.size() > 0) {
                    selectTeamImagePath = selectList.get(0).getCutPath();
                    RequestOptions requestOptions = new RequestOptions();
                    requestOptions.circleCrop();
                    Glide.with(context).load(selectTeamImagePath).apply(requestOptions).into(team_logo);
                } else {
                    selectTeamImagePath = "";
                    Glide.with(context).load(R.mipmap.img_add).into(team_logo);
                }
                break;
        }
    }
