...
public class RefereePresenter {
    ...
    //带参基本请求方法
    public void getAllRefereeByStatus(Map<String,String> params){
        if (params==null)return;
        Log.e("开始请求","p-->"+params.toString());
        new ApiUtil(context)
                .getServer(ApiService.class)
                //记得更改请求接口数据
                .getRefereeByStatus(params)
    ...
    //带参基本请求方法
    public void updateReferee(Map<String,String> params){
        if (params==null)return;
        Log.e("开始请求","p-->"+params.toString());
        new ApiUtil(context)
                .getServer(ApiService.class)
                //记得更改请求接口数据
                .updateReferee(params)
                .subscribeOn(Schedulers.io())//后台处理线程
                .observeOn(AndroidSchedulers.mainThread())//指定回调发生的线程
                .subscribe(new Observer<ResultModel<RefereeModel>>() {
                    @Override
                    ...
                    public void onNext(ResultModel<RefereeModel> resultModel) {
                        //更新视图
                        RefereeContract refereeContract = (RefereeContract)view;
                        refereeContract.updateRefereeResult(true,resultModel);
                        //view.success(resultModel);
                    }
                    @Override
                    public void onError(Throwable e) {
                        ...
                        e.printStackTrace();
                        RefereeContract refereeContract = (RefereeContract)view;
                        refereeContract.updateRefereeResult(false,e);
                        //view.failed(e);
                    }
                    @Override
                    public void onComplete() {
